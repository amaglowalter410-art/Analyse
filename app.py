    ...
    try:
        ...
        with tab2:
            ...
            if not l_dup.empty :
                ...
                st.code("Aucune ligne dupliquée n'est détecctée")
            ...
            # Ne conserver que les colonnes numériques pour les graphiques
            numeric_df = df.select_dtypes(include=['number'])
            if not numeric_df.empty:
                
                # Tracer l'histogramme avec matplotlib et passer la figure à st.pyplot
                axes = numeric_df.hist(bins=50, figsize=(20, 10))
                fig = axes.flatten()[0].get_figure()
                st.pyplot(fig)
            else:
                st.info("Aucune colonne numérique n'est disponible pour l'affichage des graphiques.")
        ...
        with tab3:
            ...
            if st.button("Lancer le Nettoyage", type="primary"):
                with st.spinner("Nettoyage en cours..."):
                    ...
                    st.download_button(
                        label="📥 Télécharger le jeu de données nettoyé",
                        data=csv,
